...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    u_s_a = us_state_abbrev
    return u_s_a[state_name] if state_name in u_s_a.keys() else NOT_FOUND
...
def get_longest_state(data):
    """Receives data, which can be the us_state_abbrev dict or the states
       ...
       of the string"""
    return max(data.keys(), key=len) if type(data) == dict \
        else max(data, key=len)
